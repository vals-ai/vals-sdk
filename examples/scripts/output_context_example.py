...
def model_with_files_and_context(input_text: str, files: dict, context: dict) -> OutputObject:
    ...
    # Generate response
    response = f"Processed {len(files)} files with context: {context.get('task', 'unknown')}"
